...
| ~~Site 502s after a normal deploy, only fixed by manually restarting nginx~~ (fixed 2026-07-29) | `nginx/conf.d/stockbot.conf` used a bare `proxy_pass http://api:8000` — nginx resolves that hostname to a Docker bridge-network IP **once, at its own startup**, and caches it forever. Every deploy rebuilds and recreates the `api`/`frontend` containers with new IPs, but nginx itself never restarts (its image doesn't change), so it kept routing to the now-dead old IP. Fixed by adding `resolver 127.0.0.11 valid=10s;` (Docker's embedded DNS) and moving the upstream into a `set $upstream ...; proxy_pass http://$upstream;` variable — this forces nginx to re-resolve per-request instead of once at startup. Verified with disposable containers: swapping the upstream's IP was picked up within the 10s cache window with zero nginx restart. If this ever recurs, check whether `nginx/conf.d/stockbot.conf` still has the `resolver` line before assuming it's a new bug. |
